...
- **Non-docker build machines** For AIX we do not use containers for building. The builds for those machines are named `build-` and are set up with ansible in the same way as the test machines.
...
- Build agents are either:
  - labelled as [build](https://ci.adoptium.net/label/build/) if the machine configured and used directly for building (AIX)
...
- Various github actions checks triggered automatically on each PR to run the playbooks on macos, build the CentOS6 and Alpine3 build dockerfiles
