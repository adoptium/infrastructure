...
if [[ $Total_Disk_in_GB -le "75" ]]; then
	...
	chfs -a size=+$unall_disk_in_GB"G" /tmp
fi 
#
#############################################################
...
#
#################################
# If disk is greater than 125GB #
#################################
if [[ $Total_Disk_in_GB -gt "125" ]]; then 
	##########
	# root / #
	##########
	...
	if [[ $unall_disk_in_GB -gt "100" ]]; then
		# Limit tmp to 100, leave the rest unallocated
		# If tmp is already larger than 100 leave it as is
		if [[ $cur_tmp -lt "100" ]]; then
			add_tmp=`expr 100 - $cur_tmp`
